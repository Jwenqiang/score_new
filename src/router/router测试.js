...
if(u.indexOf('aplus') < 0){
	router.beforeEach((to, from, next) => {
		var login = store.state.userInfo;
		console.log(this)
		const nowDate=new Date().getTime();
		if(login){
			var endTime=new Date(login.date).getTime();
		}else{
			var endTime=0;
		}
		console.log(endTime)
		console.log("过期："+nowDate>endTime)
		// debugger;
		if(to.meta.title){
			document.title=to.meta.title
		}
		if(to.name == 'error'||to.name == 'days'||to.name == 'hh'||to.name == 'h5New'||to.name == 'school'||to.name == 'schoolAll'||to.name == 'call'||to.name == 'zxInfo'||to.name == 'rules'||to.name == 'rulesAll'){
			 next(); // 跳转  不需要登录
		  ...
		  // 未登陆且要跳转的页面是登录页
		  next(); // 跳转
		}else if (login && nowDate>endTime) {
			store.commit('clearLogin');//清掉登录数据
			const oldUrl=window.location.href;
			sessionStorage.setItem('toName',oldUrl);
		  // 已登录且要跳转的页面是登录页
		  next({
