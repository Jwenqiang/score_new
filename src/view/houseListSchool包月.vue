<template>
	<div class="house" style="padding-bottom: 0.2rem;" :class="showModel?'ohide':''">
		...
		<div class="aModel modelMsg" :class="showModel?'animationMsg':''">
		  ...
		  <!-- 竞拍房源 -->
		  <!-- 竞拍学校 -->
		  <div class="aMsg" style="height: 7.2rem;">
		    ...
		    <div class="clear" style="padding-bottom: 0.2rem;border-bottom: 1px solid #ededed;">
		    	<div class="jrl" style="height: 1.5rem;">
		    		<img :src="sSchool.ImagePath" width="100%" style="height: 100%;width: auto;"/>
		    	</div>
		    	<div class="jrr">
	...
	export default{
		...
		watch:{
			scrollTop(newValue, oldValue) {//滚动分页
				...
				if(this.schoolCount>this.schoolSize){
					if(sTop>0){
						if(height-100<sTop+clientHeight&&this.load){
							  // console.log('监听成功','到达底部')
							if(this.schoolSize<this.schoolCount){
								this.schoolSize=Number(this.schoolSize)+10; 
							}
							setTimeout(()=>{
								this.loadOver=true
							},1000)
			...
			setCarSchool(){
				...
				return new Promise((resolve)=>{
						this.$axios({
							...
							headers:this.header_token,
							data:{
								  "AdName": this.sSchool.GscopeName,
								    "RoomCount":this.sSchool.EstateNumber,
								    "HallCount": this.sSchool.SaleNumber,
								    "EstateCode": "",
								    "Garea": 0,
								    "SalePrice": 0,
								    "AdType": 5,
