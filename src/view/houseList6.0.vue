<template>
	<div class="house houseList" style="padding-bottom: 0.2rem;" :class="showModel?'ohide':''">
		<template v-if="houseType=='h'">
		...
		<div class="earnAll">
			<div class="jjrSection">
				<div v-if="house.length>0">
					<div class="clear jjrL" v-for="item in house" :key="item.ZyHouseOutputDto.AdsNo">
						<div class="jrl">
							<img :src="item.ZyHouseOutputDto.FullImagePath" width="100%" @click="shareHouse(item.ZyHouseOutputDto.AdsNo,jjrNum)"/>
						</div>
						<div class="jrr">
							<div @click="shareHouse(item.ZyHouseOutputDto.AdsNo,jjrNum)">
								...
								<p class="jrbtn">
									...
									<mt-button type="primary">分享</mt-button>
								</p>
							</div>
						</div>
					</div>
		...
		<template v-else-if="houseType=='s'">
			<div class="house hbh schoolList" style="padding-top: 1.2rem;">
					<div class="hSearch">
						<i class="mintui mintui-search"></i>
						<span class="iclear" v-if="content" @click="content=''"></span>
						<!--  @input="getTags()" -->
						<input type="text" placeholder="请输入学校名称或学校别名" v-model="content" @keyup.enter="getSchool()"/>
						<button @click="getSchool()">搜索</button>
					</div>
					<div class="earnAll">
						<div class="jjrSection">
							<div v-if="houseSchool.length>0">
								<div class="clear jjrL" v-for="item in houseSchool">
									<div class="jrl" @click="shareHouse(item.SchoolId,jjrNum)">
									...
									<div class="jrr">
										...
										<p class="jrbtn hbhBtn">
											<mt-button type="primary" class="hbShare" @click="payCschool(item,'0')">竞拍</mt-button>
											<!-- <mt-button type="primary" style="border-color: #999;color: #999;" v-else>竞拍</mt-button> -->
								...
								<p class="noList" @click="schoolSize+=10" v-if="schoolCount>schoolSize">正在加载</p>
								<p class="noList" v-else-if="schoolCount<=schoolCount&&loadOver"><span></span>&nbsp;我是有底线的&nbsp;<span></span></p>
							</div>
							<div class="none" v-if="houseSchool.length==0&&ready">暂无学校信息~</div>
						</div>
					</div>
				</div>
		</template>
	...
	export default{
		...
		methods:{
				...
				return new Promise((resolve)=>{
						...
						.then(res=>{
							...
							if(res.data.code==0){
								...
								this.maxNum=res.data.data.AdPositionList[0].MaxYuanBao;
							}else{
								Toast(res.data.msg);
							}
							// Indicator.close();
			...
			setCarSchool(){
				...
				var EstateName="";
					adplace=this.sSchool.SchoolId;
					EstateName=this.sSchool.SchoolName;
				return new Promise((resolve)=>{
						this.$axios({
							method:"post",
							url:"/AdPosition/AddPreAuctionCar",
			...
			getSchool(){
				...
				return new Promise((resolve)=>{
						...
						.then(res=>{
							...
							if(res.data.code==0){
								this.ready=true;
								this.schoolCount=res.data.data.Total;
								this.houseSchool=res.data.data.Result;
							}else{
								Toast(res.data.msg);
							}
							Indicator.close();
...
/* 	  position: fixed;
	...
	.houseT .mint-searchbar-inner .mintui-search{position: relative;top: 1px !important;font-size: 0.4rem !important;margin-right: 0.15rem;color: #999;}
