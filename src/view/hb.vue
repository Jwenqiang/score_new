<template>
  <div class="hb" :class="hbList.length>0?'hbY':''">
    <div class="hbTab">
      ...
      <label :class="hbType==2?'on':''" @click="getShopList()">海报商城<!-- <span v-if="(showZp&&hbId==2)||(showJr&&hbId==3)"></span> --></label>
      </a>
    </div>
    <div class="hbList">
      <!-- 我的 -->
      <div class="myHb" v-if="hbType==1">
        <ul class="clist" v-if="hbList.length>0">
		 <template v-if="hbId==7">
			  <li v-for="(item,index) in hbList" :key="item.MyTemplateId" @click="setCj(item.MyTemplateId,item.TemplateName,item.EmpNo,item.EmpCnName)">
...
<!-- 	<nut-popup style="border-radius: 0.12rem;" v-model="showTag">
	      ...
	      <div class="cjM">
			  <p class="cjInput">
				  ...
				  <ul v-show="cjList.length>0">
					  <li v-for="item in cjList" :key="item.EmpNo" @click="selectName(item.EmpCnName,item.EmpNo)">{{item.EmpCnName}}({{item.EmpNo}}) {{item.DeptName}}</li>
				  </ul>
			  </p>
			  <p class="cjInput"><input type="text" readonly placeholder="成交时间" @click="isVisible1=true" v-model="cjTime"/></p>
			  <p class="cjInput"><input type="text" placeholder="成交小区" v-model="cjXq"/></p>
			  <p class="cjInput"><input type="text" placeholder="成交物业" readonly  @click="isVisible=true" v-model="cjWy"/></p>
		  </div>
	      <div class="miBtn">
	...
	<nut-popup style="border-radius: 0.12rem;" v-model="over">
		<div class="overHb" style="width: 5.6rem;height: 3.4rem;padding: 0.8rem 0.3rem 0;text-align: center;color: #000;">
			<p>您已获得{{overNum}}元宝，已达活动奖励上限!</p>
			<button @click="over=false" style="width: 2.8rem;height: 0.7rem;margin: 0.8rem auto 0;background: linear-gradient(0deg, #CC2720, #FF5B54);border-radius: 0.35rem;color: #fff;font-weight: 550;">我知道了</button>
		</div>
	</nut-popup>
	<!--  -->
	<nut-popup style="border-radius: 0.12rem;" v-model="shareError">
		<div class="overHb" style="width: 5.6rem;height: 3.4rem;padding: 0.8rem 0.3rem 0;text-align: center;color: #000;">
			...
			<button @click="shareError=false" style="width: 2.8rem;height: 0.7rem;margin: 0.8rem auto 0;background: linear-gradient(0deg, #CC2720, #FF5B54);border-radius: 0.35rem;color: #fff;font-weight: 550;">我知道了</button>
		</div>
	</nut-popup>
  </div>
</template>
...
<script>
  ...
  import { uToken } from "@/global/token.js";
...
<style scoped>
  .hb{background-color: #E6E6E6;min-height: 100vh;}
  /* .hbY{background-color: #fff;} */
  .hbTab{text-align: center;padding: 0.4rem 0;font-size: 0.32rem;color: #333;position: fixed;top: 0;left: 0;width: 100%;z-index: 7; background-color: #fff; box-shadow: 0px 2px 6px 2px rgb(0 0 0 / 6%); background-color: #fff;}
  .hbTab a{display: inline-block;background-color: #fff;border-radius: 0.35rem;box-shadow: 0 0 2px #ccc;}
  .hbTab label{display: inline-block;width: 2.6rem;height: 0.7rem;margin: 0 0.2rem 0 0;line-height: 0.7rem;color: #333;position: relative;}
  ...
  /** 海报大图预取 */
  .poster-prefetch { position: absolute; width: 1px; height: 1px; left: -100%; top: -100%; opacity: 0; }

  /** 海报图片加载错误时处理 */
  .bigHbError { display: flex; flex-direction: column; background: none; }
  .bigHbError > .bigS, .bigHbError > .bigHc { display: none; }
  .bigHbError > .poster-error { flex: 1; display: flex; flex-direction: column; justify-content: center; align-items: center;}
  .bigHbError .poster-error-icon { color: ref; margin-bottom: .3rem; }
  .bigHbError .poster-error-text { color: #999; margin-bottom: 3rem; }

  .bigHb img{display: block;width: 100%;}
  .bigHb label{width: 0.6rem;height: 0.6rem;background: url(../assets/img/r-close.png) right top no-repeat;background-size: 0.4rem;position: absolute;right: 0.2rem;top: 0.2rem;}
  .hbNone{padding-top: 6rem; height: 100%;background: url(../assets/img/hb-none.png) center 1.6rem no-repeat;background-size: 4rem;text-align: center;font-size: 0.28rem;color: #666;}
  .nhb{height: 2rem;width: 100%;position: absolute;bottom: 1.4rem;left: 0;padding: 0.26rem 0.39rem;background-color: #fff;}
  .nhb label{width: 1.48rem;float: left;display: inline-block;height: 1.48rem;overflow: hidden;}
  .nhbR{width: 5.2rem;float: left;text-align: left;padding-left: 0.2rem;}
  .nhbR h5{font-size: 0.32rem;font-weight: 400;color: #333;}
...
.myIntr{width: 6.2rem;height: 5.6rem;background-color: #fff;}
  ...
  @media screen and (min-device-height : 812px){
    .bigHb{width: 6.7rem;min-height: 12rem;}
  }
  ...
  .sModel{
    ...
    .hb .bRead{
      ...
      .hb .bRead p{height: 1.74rem;}
     .hb .bRead strong{font-size: 1.33rem;}
	...
	.myCj{height: 8.2rem;}
	.cjM{padding-top: 0.1rem;}
	.cjInput input{width: 5.4rem;height: 0.8rem;background-color: #EDEDED;border-radius: 0.4rem;border: 0;padding-left: 0.4rem;font-size: 0.3rem;}
	.cjInput{text-align: center;margin-bottom: 0.3rem;position: relative;}
	.cjInput ul{width: 5.4rem;position: absolute;top: 0.8rem;left: 0.1rem;z-index: 3;padding: 0.2rem 0.2rem 0.2rem 0.3rem;background-color: #f5f5f5;}
	.cjInput ul li{display: block;margin-bottom: 0.2rem;line-height: 0.6rem;font-size: 0.24rem;white-space: nowrap;overflow: hidden;text-overflow: ellipsis;text-align: left;}
	.fixR{position: fixed;top: 63%;right: 0;z-index: 9;}
	.fixR img{width: 1.2rem;}
	
	.sucHb{width: 6.4rem;height: 8.5rem;background: url(../assets/static/cjUp.png) center no-repeat;background-size: 100%;position: relative;}
	.upB{
		position: absolute;
	    bottom: 0.6rem;
	    left: 1.4rem;
	...
	.hb .popup-box{overflow-y: visible;}
	...
	.siTip{color: #fff;text-align: center;margin-top: 0.2rem;}
	.hb .tagLx span.on strong{display: block;width: 0.6rem;height: 2px;background-color: #fff;margin: 0.2rem auto;}
	.nBtn{position: absolute;top: 2.2rem;left: 0;padding: 0 0.4rem;width: 100%;height: 0.64rem;}
