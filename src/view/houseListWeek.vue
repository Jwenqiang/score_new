<template>
	<div class="house houseList week" style="padding-bottom: 0.2rem;" :class="showModel?'ohide':''">
			...
			<div class="jjrSection">
				<div v-if="house.length>0">
					<div class="clear jjrL" v-for="item in house" :key="item.ZyHouseOutputDto.AdsNo">
						<div class="jrl">
							<img :src="item.ZyHouseOutputDto.FullImagePath" width="100%"/>
						</div>
						<div class="jrr">
							...
							<p class="jrbtn">
								<mt-button type="primary" @click="shareHouse(item.ZyHouseOutputDto.AdsNo,jjrNum)">分享</mt-button>
								<mt-button type="primary" @click="payC(item.ZyHouseOutputDto,'0',item.IsSelf)" v-if="item.IsAuctionBtnClick&&item.IsSelf">包周</mt-button>
								<mt-button type="primary" style="background: none;color: #999;border: 1px solid #999;" v-else>包周</mt-button>
							</p>
						</div>
					</div>
				...
				<div class="none" v-else-if="house.length==0&&ready" style="color: #fff;">您尚未发布房源广告，请先在A+发布房源广告~</div>
			</div>
		</div>
		</mt-loadmore>
		<!-- 竞拍模块 -->
		<div class="aModel modelMsg" :class="showModel?'animationMsg':''">
		  ...
		  <!-- 包周提示 -->
		  <div class="weekTip" :class="showModel&&!showPay?'showWeekTip':''" v-show="!showPay">
			  <button @click="showPay=true">我知道了</button>
		  </div>
		  <!-- 竞拍房源 -->
		  <div class="aMsg" v-show="showPay">
...
/* 	  width: 100%;
	...
	background: linear-gradient(134deg, #FB6F52 0%, #F3240A 100%);border-radius: 3px;font-size: 0.32rem;}
		...
		.week .jrbtn button{width: 1.6rem;}
		.week .jrbtn button:first-child{margin-right: 0.4rem;color: #6089D9;border-color: #6089D9;}
...
<style scoped>
	...
	.hBan{height: 4.27rem;width: 100%;background: url(../assets/static/weekT.png) center no-repeat;background-size: 100%;position: relative;}
	.bBy{
		display: block;
		position: absolute;
		top: 0.1rem;
